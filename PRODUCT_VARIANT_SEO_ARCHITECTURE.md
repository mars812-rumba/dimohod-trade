...
Dimohod Trade. Первые импортированные прайсы уже переведены из модели “один SKU = один Product” в
модель Product → Variant/SKU.
...
- физически в `products`: `6340` строк;
- активных logical products: `40`;
- legacy products выключено через `is_active = false`: `6300`;
- всего SKU: `6341`;
- импортированных SKU с variant slug и variant-полями: `6339`.

Перед следующей массовой загрузкой новые импорты должны сразу писать данные в Product → Variant/SKU,
а не создавать отдельный logical Product на каждую цену/диаметр.
