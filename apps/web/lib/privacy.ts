export const privacyPolicyPath = "/privacy";
export const personalDataConsentPath = "/consent-personal-data";
export const cookiePolicyPath = "/cookie-policy";

export const personalDataConsentVersion = "2026-08-11";

export const operator = {
  name: "ООО «ДЫМОХОД-ТРЕЙД ПЛЮС»",
  address: "192029, г. Санкт-Петербург, ул. Хрустальная, д. 11, литера Б, помещ. 1-Н: 17",
  email: "info@dimohod-trade.pro",
  phone: "+7 (965) 075-65-55",
  phoneHref: "tel:+79650756555",
  ogrn: "1177847018216",
  inn: "7811635572",
};
