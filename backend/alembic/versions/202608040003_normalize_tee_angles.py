"""Merge tee families split by missing angle units.
...
from app.db.catalog_item_rules import confirmed_tee_angle
...
PREVIOUS_STATE_KEY = "_tee_angle_previous_state"
...
def tee_angle(name: str, attributes: dict | None) -> int | None:
    logical_name = (attributes or {}).get("logical_item_name")
    for value in (logical_name, name):
        if not value:
            continue
        confirmed = confirmed_tee_angle(str(value))
        if confirmed is not None:
            return confirmed
        match = re.search(r"к\s*/\s*о\s+(45|90)\s*(?:гр|°)?", str(value), re.IGNORECASE)
        if match:
            return int(match.group(1))
    return None


def normalized_tee_sku_name(name: str, angle: int) -> str:
    pattern = re.compile(
        rf"^тройник\s+с\s+к\s*/\s*о\s+{angle}\s*(?:гр|°)?",
        re.IGNORECASE,
    )
    return pattern.sub(f"Тройник с К/О {angle}°", name, count=1)
...
def merge_tee_category(
    ...
    rows = bind.execute(
        ...
        .where(
            ...
            products.c.product_kind == "тройник",
    ...
    for row in rows:
        angle = tee_angle(row.name, row.extra_attributes)
        if angle in {45, 90}:
            grouped[angle].append(row)
    ...
    for angle, group in grouped.items():
        ...
        for row in group:
            ...
            product_attributes["logical_item_name"] = f"Тройник с К/О {angle}гр"
            product_attributes["catalog_angle_deg"] = angle
            ...
            for sku_row in original_skus[row.id]:
                ...
                sku_attributes["angle_deg"] = angle
                ...
                bind.execute(
                    ...
                    .values(
                        ...
                        name=normalized_tee_sku_name(sku_row.name, angle),
                        slug=target_slug,
                        angle_deg=angle,
            ...
            bind.execute(
                ...
                .values(
                    name=(
                        f"{public_prefix}{prefix_separator}тройник с К/О {angle}°"
                        if row.id == canonical.id
                        else row.name
                    ),
...
def upgrade() -> None:
    ...
    replacements = {}
    replacements.update(
        merge_tee_category(bind, "odnokonturnye-troyniki", "Одноконтурный")
    )
    replacements.update(merge_tee_category(bind, "sendvich-troyniki", "Сэндвич"))
