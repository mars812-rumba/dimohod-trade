"""Reconcile outer-pipe fields with their imported price-section titles.
...
revision: str = "202608050007"
down_revision: str | None = "202608050006"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

PREVIOUS_STATE_KEY = "_outer_pipe_reconcile_previous_state"
OUTER_KEYS = (
    "outer_material",
    "outer_steel_grade",
...
def upgrade() -> None:
    ...
    for sku_id, raw_attributes in rows:
        ...
        if not recovered["outer_material"] or all(
            attributes.get(key) == recovered.get(key) for key in OUTER_KEYS
        ):
            continue
        attributes[PREVIOUS_STATE_KEY] = {
            key: {
